...
function endGame() {
  if (gameRunning) {
    ...
    // Armazenar a pontuação quando o jogo terminar
    localStorage.setItem("pontuacao", score);

    // Adicionar a pontuação ao histórico de pontuações
    var historicoPontuacoes =
      JSON.parse(localStorage.getItem("historicoPontuacoes")) || [];
    historicoPontuacoes.push(score);
    localStorage.setItem(
      "historicoPontuacoes",
      JSON.stringify(historicoPontuacoes)
    );
  }
}
